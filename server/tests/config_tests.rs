...
impl Cfg {
    ...
    fn ws(&mut self, name: &str) -> ChildPath {
        ...
        self.workspaces.push((S!(name), dir.path().sanitize()));
        dir
    }
...
fn single_workspace_root_is_addon_path() {
    ...
    assert!(config.addons_paths().contains(&ws.path().sanitize()));
}
...
fn multiple_workspaces_only_addon_roots_detected() {
    ...
    let config = cfg.default();
    assert!(config.addons_paths().contains(&ws1.path().sanitize()));
    assert!(!config.addons_paths().contains(&ws2.path().sanitize()));
}
...
fn workspace_root_is_odoo_path() {
    ...
    let config = cfg.default();
    assert_eq!(config.odoo_path().as_ref(), Some(&ws.path().sanitize()));
}
...
fn odoo_path_detected_from_workspace_child() {
    ...
    let config = cfg.default();
    assert_eq!(config.odoo_path(), Some(odoo.path().sanitize()));
}
...
fn addons_paths_detected_from_workspace_children() {
    ...
    assert!(addons.contains(&enterprise.path().sanitize()),
        "expected {}, got {addons:?}", enterprise.path().sanitize());
    assert!(addons.contains(&themes.path().sanitize()),
        "expected {}, got {addons:?}", themes.path().sanitize());
}
...
fn full_src_folder_scenario() {
    ...
    let config = cfg.default();
    assert_eq!(config.odoo_path(), Some(odoo.path().sanitize()));
    assert!(config.addons_paths().contains(&enterprise.path().sanitize()));
    assert!(config.addons_paths().contains(&custom.path().sanitize()));
    assert!(config.addons_paths().contains(&themes.path().sanitize()));
}
...
fn direct_workspace_match_used_for_odoo_path() {
    ...
    assert_eq!(cfg.default().odoo_path().as_ref(), Some(&ws.path().sanitize()));
}
...
fn user_config_overrides_auto_detection() {
    ...
    write_odools(&src, &format!(
        "[[config]]\nname = \"default\"\nodoo_path = \"{}\"\n",
        user_odoo.path().sanitize()
    ));

    assert_eq!(cfg.default().odoo_path().as_ref(), Some(&user_odoo.path().sanitize()));
}
...
fn child_addons_detected_when_root_is_also_addon() {
    ...
    let addons = cfg.default().addons_paths();
    assert!(addons.contains(&ws.path().sanitize()));
    assert!(addons.contains(&subdir.path().sanitize()));
}
...
fn addons_detected_nested_below_immediate_children() {
    ...
    let addons = cfg.default().addons_paths();
    assert!(addons.contains(&myaddons.path().sanitize()),
        "expected nested addons dir to be detected, got {addons:?}");
    assert!(!addons.contains(&ws.child("group").path().sanitize()),
...
fn recursive_scan_prunes_modules_and_skipped_dirs() {
    ...
    let addons = cfg.default().addons_paths();
    assert!(addons.contains(&real.path().sanitize()),
        "real addons dir should be detected, got {addons:?}");
    assert!(!addons.contains(&buried.path().sanitize()),
        "addons buried inside a module must be pruned, got {addons:?}");
    for skip in [".hidden", "node_modules", "__pycache__"] {
        let s = ws.child(skip).child("skipaddons").path().sanitize();
...
fn addon_path_requires_manifest() {
    ...
    write_odools(&ws, &format!(
        "[[config]]\nname = \"default\"\naddons_paths = [\"{}\"]\n",
        not_addon.path().sanitize()
    ));

    assert!(!cfg.default().addons_paths().contains(&not_addon.path().sanitize()),
...
name = "My Config"
odoo_path = "{}"
"#, community.path().sanitize()));

    let cfg = c.entry("My Config");
    assert_eq!(cfg.odoo_path().as_ref().unwrap(), &community.path().sanitize());
}
...
"#);
    ...
    assert!(c.default().addons_paths().contains(&ws.path().sanitize()));
}
...
name = "default"
addons_paths = ["{}"]
"#, child_addons.path().sanitize()));

    let paths = c.default().addons_paths();
    assert!(paths.contains(&parent_addons.path().sanitize()));
    assert!(paths.contains(&child_addons.path().sanitize()));
}
...
"#, community.path().sanitize()));
    ...
    assert_eq!(cfg.file_cache(), false);
    assert_eq!(cfg.odoo_path().as_ref().unwrap(), &community.path().sanitize());
    assert!(cfg.addons_paths().contains(&project_a.path().sanitize()));
}
...
"#);
    ...
    assert_eq!(cfg.file_cache(), false);
    assert_eq!(cfg.odoo_path().as_ref().unwrap(), &community.path().sanitize());
    assert!(cfg.addons_paths().contains(&addons.path().sanitize()));
}
...
fn template_workspacefolder_variations() {
    ...
    let cfg = c.default();
    assert!(cfg.addons_paths().iter().any(|p| p == &ws1.path().sanitize()));
    assert!(cfg.addons_paths().iter().any(|p| p == &ws2.path().sanitize()));
...
fn template_userhome_and_workspacefolder_expansion() {
    ...
    assert!(cfg.addons_paths().contains(&ws1.path().sanitize()));
}
...
fn path_case_and_trailing_slash_normalization() {
    ...
    let with_slash = ws.child("").path().sanitize();
    let without_slash = ws.path().sanitize();

    let mut normalized_addon = ws.path().sanitize();
...
fn provenance_multiple_files_and_extends() {
    ...
    assert!(cfg.scalar_sources(ConfigKey::PythonPath).unwrap().contains(&parent.path().sanitize()));
    // file_cache comes from the parent's base config (via extends).
    assert!(cfg.scalar_sources(ConfigKey::FileCache).unwrap().contains(&parent.path().sanitize()));
    // auto_refresh_delay comes from the workspace file (overrides parent).
    assert!(cfg.scalar_sources(ConfigKey::AutoRefreshDelay).unwrap().contains(&ws_f.path().sanitize()));
}
...
fn provenance_template_variable_workspacefolder() {
    ...
    assert!(addons.iter().any(|s| s.value() == &ws1.path().sanitize() && s.sources().contains(&src)));
    assert!(addons.iter().any(|s| s.value() == &ws2.path().sanitize() && s.sources().contains(&src)));
}
...
fn provenance_multiple_workspace_folders_merge_sources() {
    ...
    assert!(sources.contains(&f1.path().sanitize()));
    assert!(sources.contains(&f2.path().sanitize()));
}
...
fn no_conflict_when_config_files_point_to_same_odoo_path() {
    ...
    assert_eq!(c.default().odoo_path().as_ref().unwrap(), &ws1.path().sanitize());
}
...
fn merge_different_odoo_paths_and_addons_paths() {
    ...
    write_odools(&ws1, &format!(
        "[[config]]\nname = \"default\"\nodoo_path = \"{}\"\naddons_paths = [\"{}\", \"{}\"]\n",
        ws1.path().sanitize(), shared.path().sanitize(), ws1_addons.path().sanitize()
    ));
    write_odools(&ws2, &format!(
        "[[config]]\nname = \"default\"\nodoo_path = \"{}\"\naddons_paths = [\"{}\", \"{}\"]\n",
        ws1.path().sanitize(), shared.path().sanitize(), ws2_addons.path().sanitize()
    ));

    let expected = [
        shared.path().sanitize(),
    ...
    let sources: Vec<_> = entry
        ...
        .iter()
        .filter(|s| s.value() == &shared.path().sanitize())
...
fn addons_merge_default_merges_override_drops_parent() {
    ...
    let paths = c.default().addons_paths().clone();
    assert_eq!(paths, [child_addons.path().sanitize()].into_iter().collect::<HashSet<_>>());
}
...
fn addons_merge_override_unset_vs_empty() {
    ...
    write_odools(&ws, "[[config]]\nname = \"default\"\naddons_merge = \"override\"\n");
    let paths = c.default().addons_paths().clone();
    assert!(paths.contains(&ws.path().sanitize()));
    assert!(!paths.contains(&parent_addons.path().sanitize()));
...
fn addons_merge_override_with_auto_detect_token() {
    ...
    let paths = c.default().addons_paths().clone();
    assert!(paths.contains(&ws.path().sanitize()), "auto-detected workspace kept");
    assert!(!paths.contains(&parent_addons.path().sanitize()), "parent dropped by override");
}
...
fn addons_merge_control_key_in_parent_is_ignored() {
    ...
    let expected = [parent_addons.path().sanitize(), ws_addons.path().sanitize()]
        .into_iter().collect::<HashSet<_>>();
    assert_eq!(c.default().addons_paths().clone(), expected,
...
fn auto_detect_addons_token_combines_manual_and_detected() {
    ...
    write_odools(&ws, &format!(
        "[[config]]\nname = \"default\"\naddons_paths = [\"{}\", \"$autoDetectAddons\"]\n",
        custom.path().sanitize()
    ));

    let paths = c.default().addons_paths().clone();
    assert!(paths.contains(&custom.path().sanitize()), "manual path present");
    assert!(paths.contains(&ws.path().sanitize()), "auto-detected workspace present");
}
...
fn additional_stubs_merge_override_vs_merge() {
    ...
    write_odools(&c.temp, &format!(
        "[[config]]\nname = \"default\"\nadditional_stubs = [\"{}\"]\n",
        parent_stub.path().sanitize()
    ));

    // Override: only the child stub.
    write_odools(&ws, &format!(
        "[[config]]\nname = \"default\"\nadditional_stubs = [\"{}\"]\nadditional_stubs_merge = \"override\"\n",
        child_stub.path().sanitize()
    ));
    let stubs = c.default().additional_stubs().clone();
    assert!(stubs.contains(&child_stub.path().sanitize()), "override: child stub present");
    assert!(!stubs.contains(&parent_stub.path().sanitize()), "override: parent stub dropped");
    ...
    let stubs = c.default().additional_stubs().clone();
    assert!(stubs.contains(&child_stub.path().sanitize()), "merge: child stub present");
    assert!(stubs.contains(&parent_stub.path().sanitize()), "merge: parent stub present");
}
...
fn extra_config_file_merged_with_workspace() {
    ...
    c.with_config_file(ext.path().sanitize());

    let cfg = c.default();
    assert_eq!(cfg.file_cache(), false);
    assert_eq!(cfg.auto_refresh_delay(), 4321);
    let paths = cfg.addons_paths().clone();
    assert!(paths.contains(&ws_addon.path().sanitize()), "should contain ws_addon path");
    assert!(paths.contains(&ext_addon.path().sanitize()), "should contain ext_addon path");
}
...
fn version_variable_in_odoo_path() {
    ...
    assert_eq!(cfg.odoo_path().as_ref().unwrap(), &c.dir("18.0/odoo").path().sanitize());
}
...
fn version_from_workspace_parent_dir() {
    for (version, ws_name) in [("18.0", "ws18"), ("17.0", "ws17")] {
        ...
        assert_eq!(
            ...
            &c.dir(&format!("{version}/odoo")).path().sanitize(),
...
fn split_version_spawns_profile_per_version() {
    ...
    assert!(c.entry("root-17.0").addons_paths().contains(&v17.path().sanitize()));
    assert!(c.entry("root-18.0").addons_paths().contains(&v18.path().sanitize()));
}
...
fn base_and_detect_version_resolve_for_subpaths() {
    ...
    for toml in [&toml_base, &toml_version] {
        ...
        for ws_path in [
            vdir.path().sanitize(),
            odoo_dir.path().sanitize(),
            addon_dir.path().sanitize(),
        ] {
            let mut c2 = Cfg::new();
            ...
            let base2 = c2.temp.path().sanitize();
            ...
            assert_eq!(cfg.odoo_path().as_ref().unwrap(), &od.path().sanitize());
            assert!(cfg.addons_paths().contains(&ad.path().sanitize()));
        }
    }
    ...
    c_abs.temp.child("odools.toml").write_str(r#"
        ...
        addons_paths = [ "${base}/addon-path" ]
    "#).unwrap();
    c_abs.workspaces.push((S!("ws_abs"), v.path().sanitize()));
    ...
    c_inv.temp.child("odools.toml").write_str(r#"
        ...
        addons_paths = [ "${base}/addon-path" ]
    "#).unwrap();
    c_inv.workspaces.push((S!("ws_invalid"), v.path().sanitize()));
...
"#,
        odoo = community.path().sanitize(),
        addons = addons.path().sanitize(),
        stdlib = stdlib_dir.path().sanitize(),
        stubs = stubs_dir.path().sanitize(),
    ));

    let config = cfg.entry("full_config");
    assert_eq!(config.odoo_path().as_ref().unwrap(), &community.path().sanitize());
    assert!(config.addons_paths().contains(&addons.path().sanitize()));
    assert_eq!(config.python_path(), "python3");
    assert_eq!(config.stdlib(), stdlib_dir.path().sanitize());
    assert!(config.additional_stubs().contains(&stubs_dir.path().sanitize()));
...
fn additional_stubs_basic_path_resolution() {
    ...
    assert!(cfg.default().additional_stubs().contains(&stubs_dir.path().sanitize()));
}
...
fn additional_stubs_template_variable_resolution() {
    ...
    assert!(stubs.contains(&stubs1.path().sanitize()));
    assert!(stubs.contains(&stubs2.path().sanitize()));
}
...
fn stdlib_template_variable_resolution() {
    ...
    assert_eq!(cfg.default().stdlib(), stdlib2.path().sanitize());
}
...
fn ambiguous_workspace_name_pattern_ignored_but_general_resolves() {
    ...
    "#);
    let addons = cfg.default().addons_paths();
    assert!(!addons.contains(&ws1.path().sanitize()));
    assert!(!addons.contains(&ws2.path().sanitize()));
...
fn invalid_list_item_excluded_from_runtime_and_surfaced_in_view() {
    ...
    write_odools(&ws, &format!(r#"
        ...
        name = "default"
        addons_paths = ["{}", "{}"]
    "#, good.path().sanitize(), bad.path().sanitize()));

    let (config_map, config_file) = cfg.ok();

    // Runtime: only the valid entry is used; the invalid one is excluded.
    let addons = config_map.get("default").unwrap().addons_paths();
    assert!(addons.contains(&good.path().sanitize()));
    assert!(!addons.contains(&bad.path().sanitize()),
    ...
    let bad_path = bad.path().sanitize();
...
fn invalid_additional_stubs_item_excluded_from_runtime() {
    ...
    write_odools(&ws, &format!(r#"
        ...
        name = "default"
        additional_stubs = ["{}", "{}"]
    "#, stubs_dir.path().sanitize(), "/nonexistent/path/to/stubs"));

    // Valid path is used at runtime; the invalid one is excluded.
    let stubs = cfg.default().additional_stubs();
    assert!(stubs.contains(&stubs_dir.path().sanitize()));
...
fn invalid_odoo_path_inferred_still_shows_rejection() {
    ...
    assert_eq!(config_map.get("default").unwrap().odoo_path(), Some(ws.path().sanitize()),
    ...
    assert_eq!(op.get("value").and_then(|v| v.as_str()), Some(ws.path().sanitize().as_str()));
    let info = op.get("info").unwrap().as_str().unwrap();
    assert!(info.contains(&bad.path().sanitize()),
