...
pub fn dependencies_to_json(dependencies: &Vec<Vec<Option<PtrWeakHashSet<Weak<RefCell<Symbol>>>>>>) -> serde_json::Value {
    ...
    "arch_eval": json!({
        ...
        "arch_eval": json!(dependencies[1][1].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        }),
    "validation": json!({
        ...
        "validation": json!(dependencies[2][2].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        }),
    })
}
pub fn dependents_to_json(dependents: &Vec<Vec<Option<PtrWeakHashSet<Weak<RefCell<Symbol>>>>>>) -> serde_json::Value {
    ...
    "arch": json!({
        ...
        "validation": json!(dependents[0][2].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        }),
    "arch_eval": json!({
        "arch_eval": json!(dependents[1][0].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        "validation": json!(dependents[1][1].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        }),
    "validation": json!({
        "validation": json!(dependents[2][0].as_ref().map(|set| set.iter().map(|x| x.borrow().paths()).collect::<serde_json::Value>()).unwrap_or(json!(null))),
        }),
    })
}