...
use crate::constants::{OYarn, PackageType, SymType, Tree};
...
impl ToolAPI {
    ...
    pub fn vec_tree_to_yarn_tree(str_tree: &(Vec<String>, Vec<String>)) -> Tree {
        (str_tree.0.iter().map(|x| oyarn!("{}", x.clone())).collect::<Vec<OYarn>>(), str_tree.1.iter().map(|x| oyarn!("{}", x.clone())).collect::<Vec<OYarn>>())
    }
    ...
    fn symbol_to_json(entry: Rc<RefCell<EntryPoint>>, tree: &Tree) -> serde_json::Value {
        ...
        match typ {
            ...
            SymType::XML_FILE => {
                return symbol.borrow().as_xml_file_sym().to_json();
            },
            SymType::CSV_FILE => {
                return symbol.borrow().as_csv_file_sym().to_json();
            }
        }
    }
