...
class ConnectionManager:
    ...
    def read_lsp_message(self):
        ...
        while b"\r\n\r\n" not in buffer:
            ...
            if not chunk:
                raise ConnectionError("Distant connection closed.")
            buffer += chunk
        ...
        while len(message) < content_length:
            chunk = self.connection.recv(4096)
            if not chunk:
                raise ConnectionError("Distant connection closed while reading.")
            message += chunk
        ...
        while self.connection and not self.listening_thread.isInterruptionRequested():
            try:
                ...
                try:
                    ...
                    print(f"JSON decoding error: {message}")
                    continue
                ...
                continue  # Avoid timeout errors and keep listening
            except ConnectionError as e:
                print(f"Remote connection closed: {e}")
                break
            except Exception as e:
                print(f"Unexpected error while reading message: {e}")
                break
